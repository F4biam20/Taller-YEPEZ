...
def build_appointment_doc(appointment: AppointmentCreate) -> dict:
...
def client_appointment_filter(user: dict) -> dict:
    filters = [{"client_email": user["email"]}]
    if user.get("phone"):
        filters.append({"client_phone": user["phone"]})
    return {"$or": filters}
...
async def create_appointment(appointment: AppointmentCreate, user: dict = Depends(require_admin)):
    appt_doc = build_appointment_doc(appointment)
    await db.appointments.insert_one(appt_doc)
    return AppointmentResponse(**{k: v for k, v in appt_doc.items() if k != "_id"})

@api_router.post("/public/appointments", response_model=AppointmentResponse)
async def create_public_appointment(appointment: AppointmentCreate):
    appt_doc = build_appointment_doc(appointment)
    await db.appointments.insert_one(appt_doc)
    return AppointmentResponse(**{k: v for k, v in appt_doc.items() if k != "_id"})
...
@api_router.post("/client/appointments", response_model=AppointmentResponse)
async def create_client_appointment(appointment: AppointmentCreate, user: dict = Depends(get_current_user)):
    if user["role"] != "cliente":
        raise HTTPException(status_code=403, detail="Acceso denegado")
    appt_doc = build_appointment_doc(appointment)
    appt_doc["client_email"] = user["email"]
    appt_doc["client_name"] = appointment.client_name or user["name"]
    appt_doc["client_phone"] = appointment.client_phone or user.get("phone")
    await db.appointments.insert_one(appt_doc)
    return AppointmentResponse(**{k: v for k, v in appt_doc.items() if k != "_id"})

@api_router.put("/client/appointments/{appt_id}", response_model=AppointmentResponse)
async def update_client_appointment(appt_id: str, updates: AppointmentUpdate, user: dict = Depends(get_current_user)):
    if user["role"] != "cliente":
        raise HTTPException(status_code=403, detail="Acceso denegado")
...
@api_router.delete("/client/appointments/{appt_id}")
async def delete_client_appointment(appt_id: str, user: dict = Depends(get_current_user)):
    if user["role"] != "cliente":
        raise HTTPException(status_code=403, detail="Acceso denegado")
    appointment = await get_client_appointment_or_404(appt_id, user)
    ensure_appointment_can_change(appointment)
...
def format_client_service(service: dict) -> dict:
    current_status = SERVICE_STATUS_META.get(service.get("status"), SERVICE_STATUS_META["recibido"])
    return {
        "id": service["id"],
        "appointment_id": service.get("appointment_id"),
        "vehicle_plate": service["vehicle_plate"],
        "vehicle_model": service["vehicle_model"],
        "client_name": service["client_name"],
        ...
        "mechanic_notes": service.get("mechanic_notes"),
        "diagnosis": service.get("diagnosis"),
        "estimated_cost": service.get("estimated_cost", 0),
        "payment_status": service.get("payment_status", "pendiente"),
