...
import { Car, Clock, User, CheckCircle, ClipboardList } from "lucide-react";
...
export default function MechanicDashboard() {
  ...
  const handleProcessChange = (serviceId, status) => {
    ...
    updateDraft(serviceId, {
      status,
      progress: option?.progress ?? drafts[serviceId]?.progress ?? 0
    });
  };
    ...
    try {
      await axios.put(
        `${API}/services/${serviceId}/progress`,
        {
          ...
          mechanic_notes: draft.mechanic_notes || null
        },
        getAuthHeaders()
      );
      toast.success(draft.status === "listo" ? "¡Servicio completado!" : "Actualización guardada");
      fetchServices();
    } catch (error) {
                ...
                <div className="flex flex-col lg:flex-row lg:items-start gap-6">
                  ...
                  <div className="lg:w-80 space-y-6 p-6 bg-zinc-800/20 rounded-sm border border-zinc-800">
                    ...
                    <div className="space-y-2">
                      <p className="text-xs text-zinc-500 uppercase tracking-widest text-center">Nota para el cliente</p>
                      <Textarea
                        value={draft.mechanic_notes}
                        onChange={(e) => updateDraft(service.id, { mechanic_notes: e.target.value })}
                        placeholder="Ej: se cambió bujía, falta probar en ruta..."
                        className="bg-zinc-950 border-zinc-700 text-white min-h-[92px] resize-none"
