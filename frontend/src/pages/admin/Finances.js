...
export default function Finances() {
  ...
  const fetchData = async () => {
    try {
      ...
      // El cobro se realiza al final, cuando la moto está lista para entregar.
      setServices(servicesRes.data.filter(s => s.payment_status === "pendiente" && s.status === "listo"));
        ...
        <CardContent className="p-6">
          {services.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-zinc-600 text-sm mt-2">Los servicios aparecerán aquí cuando estén listos para entregar</p>
            </div>
          ) : (
